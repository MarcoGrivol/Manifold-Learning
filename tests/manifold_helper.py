...
def fit_plot( x, n_neighbors, n_components, label_colors ):
    ...
    x1_vals = y_isomap[:, 0]
    y1_vals = y_isomap[:, 1]
    ax1.scatter( x1_vals, y1_vals, c=label_colors, s=[2 for _ in range(len(x1_vals))] )
    ax1.set_xticklabels( [] )
    ax1.set_yticklabels( [] )
    
    x2_vals = y_lle[:, 0]
    y2_vals = y_lle[:, 1] 
    ax2.scatter( x2_vals, y2_vals, c=label_colors, s=[2 for _ in range(len(x1_vals))] )
    ax2.set_xticklabels( [] )
    ax2.set_yticklabels( [] )
    
    x3_vals = y_laplace[:, 0]
    y3_vals = y_laplace[:, 1]
    ax3.scatter( x3_vals, y3_vals, c=label_colors, s=[2 for _ in range(len(x1_vals))] )
    ax3.set_xticklabels( [] )
    ax3.set_yticklabels( [] )
    
    x4_vals = y_ltsa[:, 0]
    y4_vals = y_ltsa[:, 1]
    ax4.scatter( x4_vals, y4_vals, c=label_colors, s=[2 for _ in range(len(x1_vals))] )
    ax4.set_xticklabels( [] )
    ax4.set_yticklabels( [] )
